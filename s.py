...
def approve_or_unapprove_user(message):
    ...
    if len(cmd_parts) < 2:
        bot.send_message(chat_id, "*Invalid command format. Use /approve <user_id> <plan> <days> or /unapprove <user_id>.*", parse_mode='Markdown')
        return
    ...
    if action == '/approve':
        ...
        msg_text = f"*User {target_user_id} unapproved and reverted to free.*"
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/ALEX_Vvip/197*", parse_mode='Markdown')
